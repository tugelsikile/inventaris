<?php
$totrustot = $totrus = $totnorm = $totbaru = $total = 0;
if ($data){
	foreach($data as $val){
		$dataI['data']	= $val;
		$this->load->view('manage_items_row',$dataI);
		$totrustot = $totrustot + $val->items_rusak_total;
		$totrus = $totrus + $val->items_rusak_sedang;
		$totnorm = $totnorm + $val->items_normal;
		$totbaru = $totbaru + $val->items_baru;
		$total = $total + $val->items_stock;
	}
	//paging
	$html	= ' ';
	if ($next > 0){
...
$('.totrustot').html('<?php echo $totrustot; ?>');
$('.totrus').html('<?php echo $totrus; ?>');
$('.totnorm').html('<?php echo $totnorm; ?>');
$('.totbaru').html('<?php echo $totbaru; ?>');
