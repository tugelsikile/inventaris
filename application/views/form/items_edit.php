        ...
        <form role="form" id="form_siswa">
        	<input type="hidden" name="items_id" value="<?php echo $data->items_id;?>" />
            <div class="box-body">
                ...
                <div class="col-lg-6 col-xs-6 form-group model nopadright">
                    ...
                    <input type="text" name="model" id="model" class="form-control" placeholder="Model barang" value="<?php echo $data->items_model;?>" />
                </div>
                <div class="form-group nopadding nama">
                    <label for="nama" class="control-label">Nama Barang <span class="text-warning">*</span></label>
                    <input type="text" class="form-control" id="nama" name="nama" value="<?php echo $data->items_name;?>" placeholder="Nama Barang">
                </div>
                <div class="form-group nopadding descr">
                    ...
                    <textarea name="descr" id="descr" class="form-control" placeholder="Deskripsi Barang"><?php echo $data->items_description;?></textarea>
                </div>
                <div class="col-lg-2 col-xs-2 stock form-group nopadleft">
                	<label for="rustot" class="control-label">Rusak Total</label>
                    <input type="number" name="rustot" min="0" value="<?php echo $data->items_rusak_total;?>" id="rustot" class="form-control" />
                </div>
                <div class="col-lg-2 col-xs-2 stock form-group">
                	<label for="rusak" class="control-label">Rusak</label>
                    <input type="number" name="rusak" min="0" value="<?php echo $data->items_rusak_sedang;?>" id="rusak" class="form-control" />
                </div>
                <div class="col-lg-2 col-xs-2 stock form-group">
                	<label for="norm" class="control-label">Normal</label>
                    <input type="number" name="norm" min="0" value="<?php echo $data->items_normal;?>" id="norm" class="form-control" />
                </div>
                <div class="col-lg-2 col-xs-2 stock form-group">
                	<label for="baru" class="control-label">Baru</label>
                    <input type="number" name="baru" min="0" value="<?php echo $data->items_baru;?>" id="baru" class="form-control" />
                </div>
...
$(document).ready(function(e) {
	$('#jenis').val('<?php echo $data->items_type;?>');
	$('#kategori').val('<?php echo $data->cat_id;?>');
	$('#brand').val('<?php echo $data->brand_id;?>');
	$('#satuan').val('<?php echo $data->sat_id;?>');
	...
	$('#form_siswa').submit(function(e) {
		...
		$.ajax({
			url		: base_url + 'items/edit_data_submit',
			type	: 'POST',
			data	: $(this).serialize(),
			success	: function(dt){
