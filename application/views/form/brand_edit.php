        ...
        <form role="form" id="form_siswa">
        	<input type="hidden" name="cat_id" value="<?php echo $data->brand_id;?>" />
            <div class="box-body">
                <div class="form-group nopadding nama">
                    <label for="nama" class="control-label">Nama Merek <span class="text-warning">*</span></label>
                    <input type="text" class="form-control" id="nama" name="nama" value="<?php echo $data->brand_name;?>" placeholder="Nama Merek">
                </div>
...
$(document).ready(function(e) {
	...
	$('#form_siswa').submit(function(e) {
		...
		$.ajax({
			url		: base_url + 'brand/edit_data_submit',
			type	: 'POST',
			data	: $(this).serialize(),
			success	: function(dt){
				...
				if(typeof response != 'object') {
					...
					if (response.t == 0){
						...
						$('.nama_'+response.id).html(response.nama);
						hide_modal();
					}
				}
