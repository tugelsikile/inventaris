        ...
        <form role="form" id="form_siswa">
            <div class="box-body">
                <div class="form-group nopadding nama">
                    <label for="nama" class="control-label">Nama Peminjam <span class="text-warning">*</span></label>
                    <input type="text" class="form-control" id="nama" name="nama" value="" placeholder="Nama Peminjam">
                </div>
...
$(document).ready(function(e) {
	...
	$('#form_siswa').submit(function(e) {
		...
		$.ajax({
			url		: base_url + 'peminjam/new_data_submit',
			type	: 'POST',
			data	: $(this).serialize(),
			success	: function(dt){
				...
				if(typeof response != 'object') {
					...
					if (response.t == 0){
						...
						var peminjam_id = $('#pin_userid').val();
						$('#pin_userid').html('<option value="">--Pilih Nama Peminjam--</option>'+response.peminjam);
						$('#pin_userid').val(peminjam_id);
						$('#pin_userid').select2();
						
						$('#tablepeminjambody').append(response.html);
						hide_modal();
					}
				}
